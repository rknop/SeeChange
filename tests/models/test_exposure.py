...
def test_exposure_insert( unloaded_exposure ):
    try:
        ...
        with pytest.raises( psycopg.errors.UniqueViolation, match="duplicate key value violates unique constraint" ):
            unloaded_exposure.insert()

        # Verfiy that it yells at us if we try to insert it under a different uuid but with
        #   the same filepath
        unloaded_exposure.id = uuid.uuid4()
        with pytest.raises( psycopg.errors.UniqueViolation, match='unique constraint "ix_exposures_filepath"' ):
            unloaded_exposure.insert()
